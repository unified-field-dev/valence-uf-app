...
use crate::server::get_dashboard_platform_reads_breakdown;
...
#[component]
pub fn DashboardPlatformReadsCard(
    range_secs: Signal<i64>,
    group_by: Signal<String>,
) -> impl IntoView {
    let res = Resource::new(
        move || (range_secs.get(), group_by.get()),
        |(secs, group)| async move { get_dashboard_platform_reads_breakdown(secs, group).await },
    );

    view! {
        <Card>
            <ValenceHelpCardHeader
                title="Reads breakdown"
                description="Read rate grouped by table or database type."
            />
            <CardContent>
                <div id="valence-dashboard-reads" data-testid="dashboard-platform-reads-card">
                    <Transition fallback=move || view! { <DashboardChartSkeleton /> }>
                        {move || res.get().map(|r| match r {
                            Ok(series) if series.is_empty() => view! {
                                <Caption1>"No read metrics in this range."</Caption1>
                            }.into_any(),
                            Ok(series) => view! {
                                <Caption1>"Reads / sec by dimension"</Caption1>
                                {area_chart_from_series(&series, "reads", 280.0)}
                            }.into_any(),
